...
if (!RNFileAccess) {
    ...
    setTimeout(() => {
        console.warn(
            'RNFileAccess: Native module is not available, make sure you have finished the installation process and rebuilt your app'
        );
    }, 0);
} else if (!RNFileAccess.startAccess) {
    // Use a timeout to ensure the warning is displayed in the YellowBox
    setTimeout(() => {
        console.warn(
            'RNFileAccess: Native module is obsolete, you may not have rebuilt your app after upgrading the library'
        );
    }, 0);
}
...
//     console.warn(
//       `RNFileAccess: Android API level ${Platform.Version} does not support multiple types, falling back to */*`
//     );
//   }
...
//   if ('mode' in opts && !['import', 'open', 'export', 'move'].includes(opts.mode)) {
...
//   RNFileAccess.releaseSecureAccess(uris);
// }
...
export default class DocumentPicker {
    ...
    static async startAccess({ url, bookmark }) {
        if (Platform.OS !== 'ios') {
            return false;
        }
        try {
            let result = await RNFileAccess.startAccess({ url });
            if (result !== 'success') {
                result = await RNFileAccess.startAccess({ url, bookmark });
            }
    ...
    static async stopAccess({ url }) {
        if (Platform.OS !== 'ios') {
            return false;
        }
        try {
            const result = await RNFileAccess.stopAccess({ url });
            return result === 'success'
